...
export const AdminCoursePage: React.FC = () => {
  ...
  const [courseStructure, setCourseStructure] = useState<CourseStructure | undefined>(
    initialStructure
  );
  ...
  // Content Authoring Modal state (New or Edit)
  ...
  const handleSaveContentItem = (savedDetail: ContentItemDetail, isNew: boolean) => {
    if (!contentModalState || !courseStructure) return;
    const { moduleId, submoduleId } = contentModalState;

    setCourseStructure((prev) => {
      if (!prev) return prev;

      const summaryItem: ContentItemSummary = {
        id: savedDetail.id,
        title: savedDetail.title,
        type: savedDetail.type,
        marks: savedDetail.marks,
        status: savedDetail.status,
        durationMinutes: savedDetail.durationMinutes
      };
      ...
      const updatedModules = prev.modules.map((m) => {
        ...
        return {
          ...
          submodules: m.submodules.map((sub) => {
            if (sub.id !== submoduleId) return sub;

            if (isNew) {
              return {
                ...sub,
                contentItems: [...sub.contentItems, summaryItem]
              };
            } else {
              return {
                ...sub,
                contentItems: sub.contentItems.map((ci) => {
                  if (ci.id === savedDetail.id) {
                    oldItemMarks = ci.marks;
                    return summaryItem;
                  }
                  return ci;
                })
      ...
      const newMaxScore = isNew
        ? prev.maxScore + savedDetail.marks
        : prev.maxScore - oldItemMarks + savedDetail.marks;
      ...
      courseApi.saveCourseStructure(activeCourseId, updatedStructure);
  ...
  const handleDeleteContentItem = (
    ...
    e.stopPropagation();
    if (!window.confirm(`Delete "${itemTitle}" from this submodule?`)) return;
    ...
    setCourseStructure((prev) => {
      ...
      const updatedModules = prev.modules.map((m) => {
        ...
        return {
          ...
          submodules: m.submodules.map((sub) => {
            if (sub.id !== submoduleId) return sub;
            const target = sub.contentItems.find((ci) => ci.id === itemId);
            if (target) removedMarks = target.marks;
            return {
              ...sub,
              contentItems: sub.contentItems.filter((ci) => ci.id !== itemId)
            };
          })
        };
      });
      ...
      const updatedStructure: CourseStructure = {
        ...prev,
        maxScore: Math.max(0, prev.maxScore - removedMarks),
        modules: updatedModules
      };
  ...
  const handleDeleteSubmodule = (
    ...
    e.stopPropagation();
    if (!window.confirm(`Delete submodule "${subTitle}" and all its content?`)) return;
    ...
    setCourseStructure((prev) => {
      ...
      const updatedModules = prev.modules.map((m) => {
        if (m.id !== moduleId) return m;
        const targetSub = m.submodules.find((s) => s.id === submoduleId);
        if (targetSub) {
          removedScore = targetSub.contentItems.reduce((acc, ci) => acc + (ci.marks || 0), 0);
        }
        return {
          ...m,
      ...
      const updatedStructure: CourseStructure = {
        ...prev,
        totalSubmodules: Math.max(0, prev.totalSubmodules - 1),
        maxScore: Math.max(0, prev.maxScore - removedScore),
        modules: updatedModules
      };
  ...
  const handleDeleteModule = (e: React.MouseEvent, moduleId: string, modTitle: string) => {
    e.stopPropagation();
    if (!window.confirm(`Delete module "${modTitle}" and all its submodules?`)) return;

    setCourseStructure((prev) => {
      if (!prev) return prev;
      const targetMod = prev.modules.find((m) => m.id === moduleId);
      const subsCount = targetMod?.submodules.length || 0;
      let removedScore = 0;
      targetMod?.submodules.forEach((sub) => {
        removedScore += sub.contentItems.reduce((acc, ci) => acc + (ci.marks || 0), 0);
      });

      const updatedStructure: CourseStructure = {
        ...prev,
        totalModules: Math.max(0, prev.totalModules - 1),
        totalSubmodules: Math.max(0, prev.totalSubmodules - subsCount),
        maxScore: Math.max(0, prev.maxScore - removedScore),
    ...
    <div className="h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans selection:bg-blue-600 selection:text-white overflow-hidden">
        ...
        <main className="flex-1 w-full p-4 lg:p-6 2xl:p-8 flex flex-col min-h-0 overflow-hidden">
          {isStructureLoading ? (
            ...
            <div className="w-full h-full min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-5 xl:gap-7 items-stretch">
              {/* Left Column (8-9 cols): Island 1 (Hero Stats) + Island 2 (Curriculum / Members) */}
              <div className="lg:col-span-7 xl:col-span-8 2xl:col-span-9 flex flex-col min-h-0 h-full gap-4">
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl shadow-xs flex flex-col flex-1 min-h-0 overflow-hidden">
                      ...
                      {courseStructure?.modules.map((mod, idx) => {
                        ...
                        return (
                          <div key={mod.id} className="py-2">
                            ...
                            {isOpen && (
                              <div className="pl-6 pr-2 pt-2 space-y-3">
                                {mod.submodules?.map((sub, sIdx) => (
                                  <div
                                    key={sub.id}
                                    className="p-3.5 bg-white border border-slate-200/80 rounded-xl hover:border-blue-200 transition-all space-y-3 shadow-xs"
                                  >
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        <button
                                          type="button"
                                          onClick={(e) =>
                                            handleDeleteSubmodule(e, mod.id, sub.id, sub.title)
                                          }
                                    ...
                                    <div className="space-y-1.5">
                                      ...
                                      {(!sub.contentItems || sub.contentItems.length === 0) && (
                                        <div className="py-2.5 px-3 text-center border border-dashed border-slate-200 rounded-lg bg-slate-50/60">
                                          <p className="text-[11px] text-slate-400">
                                            No lessons yet. Add a video, MCQ quiz, or coding
                                            challenge below.
                                          </p>
                                        </div>
                                      )}
                                    ...
                                    <div className="pt-2 flex items-center justify-between flex-wrap gap-2 border-t border-slate-100">
                                      ...
                                      <div className="flex items-center gap-1.5 flex-wrap">
                                        ...
                                        <button
                                          ...
                                          className="px-2.5 py-1 rounded-lg text-xs font-bold bg-purple-50 hover:bg-purple-100 text-purple-700 transition-colors flex items-center gap-1 cursor-pointer"
                                          title="Add MCQ Assessment"
                                        >
                                        ...
                                        <button
                                          ...
                                          className="px-2.5 py-1 rounded-lg text-xs font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 transition-colors flex items-center gap-1 cursor-pointer"
                                        ...
                                        <button
                                          ...
                                          className="px-2.5 py-1 rounded-lg text-xs font-bold bg-amber-50 hover:bg-amber-100 text-amber-700 transition-colors flex items-center gap-1 cursor-pointer"
                                          title="Add Lecture Notes or Reference Document"
                                        >
                  ...
                  {activeTab === 'members' && (
                    <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 pr-3 space-y-3">
                  ...
                  {activeTab === 'grades' && (
                    <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 pr-3">
              ...
              {/* Right Column (3-4 cols): Island 3 (Class Leaderboard & Announcement) Matching LeaderboardPanel */}
              <div className="lg:col-span-5 xl:col-span-4 2xl:col-span-3 flex flex-col min-h-0 h-full gap-4 overflow-y-auto custom-scrollbar pr-1">
                {/* Live Leaderboard */}
      ...
      {contentModalState && (
        <AdminContentItemModal
          isOpen={Boolean(contentModalState.isOpen)}
          item={contentModalState.item}
          initialType={contentModalState.initialType}
          moduleId={contentModalState.moduleId}
          submoduleId={contentModalState.submoduleId}
          moduleTitle={contentModalState.moduleTitle}
          submoduleTitle={contentModalState.submoduleTitle}
